...
int AI::alphaBetaTree(int layer, float depthLeft, int alpha, int beta, int endOfLastMove)
{
    ...
    if(depthLeft <= 1 && useFutilityPruning)
    {
        // futility pruning
        return futilityPruning(layer, depthLeft, alpha, beta, endOfLastMove, hash);
    }
...
int AI::futilityPruning(int layer, float depthLeft, int alpha, int beta, int endOfLastMove, Node* hash)
{
    ...
    else if(hash != nullptr && hash->depthSearched >= depthLeft && hash->betaSearchedAt >= beta)
    {
        // returned hashed value
        return hash->value;
    }
    else if(depthLeft <= 0 || layer >= maxNumberOfPly-3)        // gives an irrational cushion to prevent running out of memory in the array
    {
        // evaluate
        return evalPosition(layer, endOfLastMove, hash, beta, depthLeft);
    }
    ...
    if(layer%2 == 0)
    {
        ...
        int currentBoardValue = evalPosition(layer, endOfLastMove, hash, beta, depthLeft);
        orderMoves(layer, hash, depthLeft, alpha, beta);
        if(currentBoardValue+futilityMargin < alpha)
        {
            // this move is very unlikely to raise alpha, so we can (almost) safely just return its value
            // to be safe, we still search captures that could raise alpha
            score = currentBoardValue;
            for(i=500*layer; i<moveLength[layer]; i++)
            {
                if(board.b[move[moveOrder[i]].to] != 0 && -1*evaluator.pieceToValue(board.b[move[moveOrder[i]].to]) > futilityMargin)
                {
                    // capture that could raise us above futilityMargin
                    bonusDepth = getExtension(layer, move[moveOrder[i]], depthLeft, inCheck, i-500*layer);
                    value = -1*alphaBetaMakeMove(move[moveOrder[i]], layer+1, depthLeft+bonusDepth-1, -1*beta, -1*alpha);
                    if(value>score)
                    {
    ...
    else
    {
        ...
        if(currentBoardValue + futilityMargin < alpha)
        {
            ...
            for(i=500*layer; i<moveLength[layer]; i++)
            {
                if(board.b[move[moveOrder[i]].to] != 0 && evaluator.pieceToValue(board.b[move[moveOrder[i]].to]) > futilityMargin)
                {
                    // capture that could raise us above futilityMargin
                    bonusDepth = getExtension(layer, move[moveOrder[i]], depthLeft, inCheck, i-500*layer);
                    value = -1*alphaBetaMakeMove(move[moveOrder[i]], layer+1, depthLeft+bonusDepth-1, -1*beta, -1*alpha);
